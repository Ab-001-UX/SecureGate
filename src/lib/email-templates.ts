export function getVerificationEmailHtml(verifyLink: string): string {
  return `
...
export function getExistingUserSignUpHtml(loginLink: string): string {
  return `
    <div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
      ...
      <a href="${loginLink}" style="display: inline-block; padding: 12px 24px; background-color: #0f172a; color: #ffffff; text-decoration: none; border-radius: 6px; font-weight: bold; margin-top: 15px;">Go to Login</a>
    </div>
  `;
}

export function getAlreadyVerifiedEmailHtml(loginLink: string): string {
  return `
    <div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
